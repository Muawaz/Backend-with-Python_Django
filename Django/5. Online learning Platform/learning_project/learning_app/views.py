...
class OnlineCourseApi(View):
    ...
    def post(self, request, *args, **kwargs):
            ...
            if serializer.is_valid():
                serializer.save()
                return JsonResponse({'message': 'Student updated successfully', 'data': python_data}, status=200)
            json_data = JSONRenderer().render(serializer.errors)
            return HttpResponse(json_data, content_type='application/json')
    
@method_decorator(csrf_exempt, name='dispatch')
class OnlineStudentApi(View):
    def get(self, request, *args, **kwargs):
        # def get_all_students(request):
        student_id = kwargs.get('id')
        if student_id:
            try:
                student = student_table.objects.get(rollNo=student_id)
